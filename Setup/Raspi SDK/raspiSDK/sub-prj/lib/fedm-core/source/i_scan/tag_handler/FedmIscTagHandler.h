...
class _FEDM_ISC_CORE_EXT_CLASS FedmIscTagHandler
{
public:
	...
	/// @param[out] usPhaseAngle Array with phase angle values
	/// @return 
	...
	int GetRSSI(unsigned char* ucNumber, unsigned char* ucStatus, unsigned char* ucRSSI, unsigned short int* usPhaseAngle);
	...
	///
	/// <b>Description</b><br>
	unsigned int GetErrorDBAddress()			{ return m_uiErrorDBAddress; }
	...
	///
	/// <b>Description</b><br>
	unsigned char GetLastISOErrorCode()		{ return m_ucISOErrorCode; }
	...
	void EnableNonAddressedMode(bool bEnable)	{ m_bNonAddressedMode = bEnable; }
	...
	///
	/// <b>Description</b><br>
	bool IsNonAddressedModeEnabled()			{ return m_bNonAddressedMode; }
	...
	/// - Without support for extended address mode<br>
	int virtual ReadMultipleBlocks(	unsigned int uiFirstDataBlock,
																unsigned int uiNoOfDataBlocks,
																unsigned int& uiBlockSize,
	...
	/// - Without support for extended address mode<br>
	int virtual WriteMultipleBlocks(	unsigned int uiFirstDataBlock,
																unsigned int uiNoOfDataBlocks,
																unsigned int uiBlockSize,
																unsigned char* pucData );
...
protected:
	// constructor
	FedmIscTagHandler(FEDM_ISCReader* pReader,	unsigned int uiTagHandlerType, FEDM_ISOTabItem* pTabItem);
	// destructor
	virtual ~FedmIscTagHandler();
...
protected:
	...
	///
	/// <b>Description</b><br>
	/// Authenticate: This command performs tag, reader, or mutual authentication.
	/// Commands [0xB0][0x35] or [0xB3][0x31]
	int Authenticate(	unsigned char ucCmdMain,
						unsigned char ucCmdSub,
	...
	///
	/// <b>Description</b><br>
	/// Challenge: This command allows an interrogator to instruct multiple tags to simultaneously yet independently precompute and store a cryptographic value.
	/// No response is send from tag on a challenge command.
	/// Commands [0xB0][0x39] or [0xB3][0x32]
	int Challenge(	unsigned char ucCmdMain,
					unsigned char ucCmdSub,
					unsigned char ucAuthMode,
					unsigned char ucCSI,
					unsigned char ucCryptoTime,
