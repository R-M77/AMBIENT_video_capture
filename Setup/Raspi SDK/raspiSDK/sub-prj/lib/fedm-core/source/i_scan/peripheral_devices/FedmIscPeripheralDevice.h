...
#define FEDM_ISC_PD_TYPE_LED_CONTROLLER		0x03
#define FEDM_ISC_PD_TYPE_UHF_PEOPLE_COUNTER	0x04

//Peripheral Device Ports
#define FEDM_ISC_PD_PORT_TYPE_NONE			0x00
#define FEDM_ISC_PD_PORT_TYPE_RS232         0x01
#define FEDM_ISC_PD_PORT_TYPE_RS4XX			0x02

#define FEDM_ISC_PD_BUS_ADDR_NONE           0x00

#define FEDM_ISC_PD_MAX_OUTPUTS             8
...
typedef struct _FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC
{
	// [0x6E] and mode=0x04
	unsigned char	ucIntErr[2];              // flag field with internal errors
	
	// [0x6E] and mode=0xE0
	unsigned char	ucLedState;               // state of the LED's
	unsigned char	ucTemperature;
	unsigned char	ucRfDetect[2];            // ADC value of RF-Detect

	bool	bIsMode0x04Read;
	bool	bIsMode0xE0Read;

	// initialization
	void Init()
	{
		memset(&ucIntErr[0], 0, sizeof(_FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC));
	}

} FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC;
...
public:
	...
	int Reset();
	virtual FEDM_ISC_PERIPHERAL_DEVICE_INFO *ReadInfo() = 0;
	FEDM_ISC_PERIPHERAL_DEVICE_INFO *GetInfo()		{ return &m_DeviceInfo; }
	virtual void BuildInfoReport(string& sReport) = 0;

	virtual FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC *ReadDiagnostic(void) = 0;
	FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC *GetDiagnostic(void) { return &m_DeviceDiagnostic; }
	virtual void BuildDiagnosticReport(string& sReport) = 0;

// protected functions
protected:
	void SetLastError(int iError);
	unsigned char DeviceTypeOfPD(void);

	bool ReadInfo0x00(unsigned char ucDevice);
	bool ReadInfo0x10(unsigned char ucDevice);
	bool ReadInfo0x60(unsigned char ucDevice);

	void Add0x00InfoText(string& sReport);
	void Add0x10InfoText(string& sReport);
	void Add0x60InfoText(string& sReport);

	bool ReadDiag0x04(unsigned char ucDevice);
	bool ReadDiag0xE0(unsigned char ucDevice);

	void Add0x04DiagText(string & sReport);
	void Add0xE0DiagText(string & sReport);
...
protected:
	...
	FEDM_ISC_PERIPHERAL_DEVICE_DIAGNOSTIC m_DeviceDiagnostic;
};
