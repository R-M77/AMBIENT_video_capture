...
protected:
	...
	FEDM_BRMTabItem(unsigned int uiTabIndex, FEDM_ISCReader* pReader);	// main constructor
...
public:
	...
	unsigned char	m_ucTagStatus;			///< Transponder Status
	unsigned char	m_ucNewAFI;				///< new Application Family Identifier
	...
	unsigned short  m_usTagCount;           ///< Number of tags
	unsigned char	m_ucRssiMax;            ///< Maximum RSSI
	unsigned char	m_ucRssiAvg;            ///< Average RSSI
	...
	unsigned char	m_ucScannerIdType;		///< 0x00: ScannerID; 0x01: DeviceID
	unsigned char	m_ucScannerIDLen;		///< Length of Scanner-ID in bytes
	unsigned char	m_ucScannerID[FEDM_ISC_MAX_SCANNER_ID_LENGTH];		///< Scanner-ID (only for Notification-Mode)
	...
	unsigned short int m_usAntPhaseAngle[FEDM_ISC_BRM_TABLE_MAX_ANTENNA];	///< Only ISC.LRU1002: Array with antenna RF phase angel of a transponder. The corresponding angle can be calculated as: PHASE_ANGLE * 360° / 4096
	//unsigned char	m_ucAntRes1[FEDM_ISC_BRM_TABLE_MAX_ANTENNA];	///< Array with antenna reserved bytes
	//unsigned char	m_ucAntRes2[FEDM_ISC_BRM_TABLE_MAX_ANTENNA];	///< Array with antenna reserved bytes
	...
	bool    m_bIsTagStatistic;  ///< Flag indicates, if item contains valid data for tag statistic
	...
	bool	m_bIsScannerID;		///< Flag indicates, if item contains valid data for Scanner-ID
