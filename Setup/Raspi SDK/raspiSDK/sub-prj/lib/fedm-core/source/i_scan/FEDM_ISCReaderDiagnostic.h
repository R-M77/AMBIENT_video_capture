...
Copyright � 2008-2016	FEIG ELECTRONIC GmbH, All Rights Reserved.
						Lange Strasse 4
						D-35781 Weilburg
...
Version       		:	04.07.00 / 26.09.2016 / M. Hultsch
						- new flags in mode 0x04 for improved LRU1002

						04.06.10 / 10.12.2014 / M. Hultsch
						- new mode 0x08
...
typedef struct _FEDM_ISC_READER_DIAGNOSTIC
{
	...
	union
	{
		...
		struct
		{
			unsigned char	bIOExpander:		1;		///< [0x6E] and Mode=0x04: Internal Error flag field A, bit 0: IO Expander error
			unsigned char	bDCOut:				1;		///< [0x6E] and Mode=0x04: Internal Error flag field A, bit 1: DC Out error
			unsigned char	bUsbImax:			1;		///< [0x6E] and Mode=0x04: Internal Error flag field A, bit 2: USB error
			unsigned char	bReserved:			5;		///< [0x6E] and Mode=0x04: Internal Error flag field A, bits 3-7: reserved
		};
		struct
		{
			unsigned char	bReserved1 :			1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, bits 0 (8) : reserved
			unsigned char	bTriggerLockStatus:		1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, bits 1 (9) : Trigger lock status
			unsigned char	bBarcode :				1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, Bit 2 (10) : Barcode
			unsigned char	bWiFiBluetooth :		1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, Bit 3 (11) : WiFi / Bluetooth
			unsigned char	bAccelerationSensor :	1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, Bit 4 (12) : Beschleunigungssensor
			unsigned char	bBattery :				1;	///< [0x6E] and Mode=0x04: Internal Error flag field A, Bit 6 (13) : Batterie
			unsigned char	bReserved2 :			2;	///< [0x6E] and Mode=0x04: Internal Error flag field A, bits 7 - 8 (14 - 15) : reserved
		};
	};
	...
	union
	{
		...
		struct
		{
			...
			unsigned char	bPeopleCounter:		1;		///< [0x6E] and Mode=0x04: Internal Error flag field B, bit 5: People Counter error
			unsigned char	bRealTimeClock:		1;		///< [0x6E] and Mode=0x04: Internal Error flag field B, bit 6: Real-Time Clock error
			unsigned char	bADC:				1;		///< [0x6E] and Mode=0x04: Internal Error flag field B, bit 7: ADC error
		};
	};
	...
	// Hardware Switches
	// [0x6E] and Mode=0x09
	unsigned char	ucMode09[16];

	// Battery Voltage
	// [0x6E] and Mode=0x10
	unsigned char	ucBatteryVoltage[2];						///< [0x6E] and Mode=0x10: Battery Voltage (index 0: MSB, index 1: LSB)
...
//	unsigned char	ucReserved3;				///< rfu
	...
	{
//		return (unsigned int)ucReaderType;
		return uiReaderType;
	}
