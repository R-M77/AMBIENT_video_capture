...
public:
	...
	friend class FedmIscTagHandler_ISO15693_STM_ST25DVxxK;
	...
	friend class FedmIscTagHandler_ISO15693_NXP_ICODE_SLIX2;
...
public:
	...
	unsigned short int m_usAntPhaseAngle[FEDM_ISC_ISO_TABLE_MAX_ANTENNA]; ///< Array with antenna phase angle
	...
	unsigned char m_ucChallenge[10];	///< UCODE DNA: challenge responded with Authenticate command
	unsigned char m_ucCryptoBlock[32];	///< UCODE DNA: cryptographic block responded with Authenticate command


	unsigned char	m_ucInfoFlag;	///< flag field from extended ISO Command [0xB0][0x2B] Get System Information
	...
	unsigned char	m_ucCommandList[6];		///< command list array from extended ISO Command [0xB0][0x2B] Get System Information
	std::vector<unsigned char> m_CryptoSuiteIDs; ///< list with supported crypto suite IDs from extended ISO Command [0xB0][0x2B] Get System Information
