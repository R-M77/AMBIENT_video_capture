...
Copyright © 2007-2017	FEIG ELECTRONIC GmbH, All Rights Reserved.
						Lange Strasse 4
						D-35781 Weilburg
...
Version       		:	04.09.01 / 24.10.2017 / M. Hultsch
						- new elements for mode 0x00

						04.07.02 / 08.02.2017 / M. Hultsch
						- new element for mode 0x15

						04.06.14 / 28.08.2015 / M. Hultsch
						- new modes: 0x15 and 0x16
...
typedef struct _FEDM_ISC_READER_INFO
{
	...
	unsigned char	ucRfcSwType;				///< [0x66] and Mode=0x00: Reader Type = SW-TYPE of protcol
//	unsigned char	ucReaderType;				///< [0x66] and Mode=0x00: Reader Type = SW-TYPE of protcol
	...
	union
	{
		struct	// CPR-Reader
		{
			...
			union
			{
				...
				struct // all modern CPR-Reader, MR102-U, PRH101
				{
	...
	// information from Bluetooth Firmware
	// [0x66] and Mode=0x0A
	unsigned char	ucBluetoothVer[3];

	// information from Barcode Firmware
	// [0x66] and Mode=0x0B
	unsigned char	ucBarcodeVer[30];
	...
	// RF-Stack firmware information
	// [0x66] and Mode=0x15
	unsigned char	ucRfStackSwVer[2];			///< [0x66] and Mode=0x15: major and minor version number of RF-Stack
	unsigned char	ucISO7816StackSwVer[2];		///< [0x66] and Mode=0x15: major and minor version number of ISO7816-Stack

	// IDT firmware information
	// [0x66] and Mode=0x16
	unsigned char	ucIdtSwVer[2];				///< [0x66] and Mode=0x16: major and minor version number of IDT
	...
	unsigned char	ucCfgReadPermissions[18];	///< [0x66] and Mode=0x40 or 0x41: flag field with readable CFG pages
	unsigned char	ucCfgWritePermissions[18];	///< [0x66] and Mode=0x40 or 0x41: flag field with writeable CFG pages
	...
	// information with MAC for Bluetooth
	// [0x66] and Mode=0x58
	unsigned char	ucBluetoothFlags;				///< [0x66] and Mode=0x58: flags
	unsigned char	ucBluetoothMac[6];				///< [0x66] and Mode=0x58: media access control
	unsigned char	ucBluetoothReserved[5];			///< [0x66] and Mode=0x58: reserved
	...
	} aPeripheralDevice[10]; // max 10 peripheral devices

	// device information #1 of RF-Controller
	// [0x66] and Mode=0x80
	...
	// device information #2 of Reader/Scanner
	// [0x66] and Mode=0x81
	char cScannerID[30];
	...
	bool bIsMode0x0ARead;	///< indicates, if data with Mode 0x0A is read
	bool bIsMode0x0BRead;	///< indicates, if data with Mode 0x0B is read
	...
	bool bIsMode0x15Read;	///< indicates, if data with Mode 0x15 is read
	bool bIsMode0x16Read;	///< indicates, if data with Mode 0x16 is read
	...
	bool bIsMode0x58Read;	///< indicates, if data with Mode 0x58 is read
	...
	bool bIsMode0x81Read;	///< indicates, if data with Mode 0x81 is read
	...
	unsigned int GetReaderType()
	{
		...
		return static_cast<unsigned int>(ucRfcSwType) | (static_cast<unsigned int>(ucReaderAssembly) << 8);
//		return static_cast<unsigned int>(ucReaderType) | (static_cast<unsigned int>(ucReaderAssembly) << 8);
	}

	void SetReaderType(unsigned int uiReaderType)
	{
		ucRfcSwType      = static_cast<unsigned char>(uiReaderType & 0xFF);
		ucReaderAssembly = static_cast<unsigned char>((uiReaderType >> 8) & 0xFF);
//		bIsMode0x00Read  = true; // CHECKME: Sollte das Flag nicht gesetzt werden ?
	}
	...
	/// Return of Scanner-ID
	std::string GetScannerID()
	{
		if (!bIsMode0x81Read) {
			return std::string();
		}
		return std::string(cScannerID);
	}
