...
public:
	...
	// see https://www.gs1.org/epcglobal/standards/mdid
...
protected:
	static float GetRssiValue(unsigned int uiReaderType, unsigned char ucRawValue, char *cUnit) {
		switch (uiReaderType) {
			...
			case FEDM_ISC_TYPE_ISCLR1002:
				*cUnit = 0;
				return static_cast<float>(ucRawValue);

			case FEDM_ISC_TYPE_ISCMU02:
			case FEDM_ISC_TYPE_ISCMU95:
			case FEDM_ISC_TYPE_ISCMU92:
			case FEDM_ISC_TYPE_ISCMRU102:
			case FEDM_ISC_TYPE_ISCLRU1002:
			case FEDM_ISC_TYPE_ISCLRU3000:
			case FEDM_RDR_TYPE_LRU500_I_BD:
			case FEDM_RDR_TYPE_LRU500_I_POE:
			case FEDM_ISC_TYPE_MAXU1002:
			case FEDM_RDR_TYPE_MAXU500_I:
			case FEDM_ISC_TYPE_ANTU_500_270_GA:
			case FEDM_ISC_TYPE_ANTU_500_270_DM:
			case FEDM_ISC_TYPE_ISCPRHD102:
#if defined(_MSC_VER)
				strncpy_s(cUnit, 4, "dBm", sizeof("dBm"));
#endif
#if defined(__GNUC__)
				strncpy(cUnit, "dBm", sizeof("dBm"));
#endif
				return -static_cast<float>(ucRawValue);

			default: // Reader does not support RSSI values
				*cUnit = 0;
				return FLT_MAX;
		}
	}

	static float GetPhaseAngleValue(unsigned int uiReaderType, unsigned short usRawValue, char *cUnit) {
		switch (uiReaderType) {
			case FEDM_ISC_TYPE_ISCLRU1002:
			case FEDM_RDR_TYPE_LRU500_I_BD:
			case FEDM_RDR_TYPE_LRU500_I_POE:
			...
			case FEDM_RDR_TYPE_MAXU500_I:
#if defined(_MSC_VER)
				strncpy_s(cUnit, 4, "deg", sizeof("deg"));
#endif
#if defined(__GNUC__)
				strncpy(cUnit, "deg", sizeof("deg"));
#endif
				return static_cast<float>((usRawValue) * 360) / 4096;

			default: // Reader does not support phase angle values
				*cUnit = 0;
				return FLT_MAX;
		}
	}
