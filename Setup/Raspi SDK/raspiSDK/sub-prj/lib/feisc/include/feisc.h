...
#define FEISC_ADJUST_AUTO_READ_MODE		0x3A
...
//									    0x89 is reserved for Transfer Parameter Set
...
#define FEISC_ISO_CUST_PROP_CMD_14443   0xB5
//										0xB8 is reserved for FeliCa transparent command
//										0xB9 is reserved for Jewel transparent command
//										0xBA is reserved for Payment command
...
// emulator errors
#define	FEISC_ERR_EMU_CMP_TX_SIZE			-4040
#define	FEISC_ERR_EMU_CMP_TX_PROT			-4041
#define	FEISC_ERR_EMU_CMP_RX_SIZE			-4042
#define	FEISC_ERR_EMU_OPEN_JSON_FAIL		-4043
#define	FEISC_ERR_EMU_JSON_READ_FAIL		-4044
#define	FEISC_ERR_EMU_JSON_READ_OVERFLOW	-4045
...
// emulator2 errors
#define	FEISC_ERR_EMU2_TEST_OVERFLOW        -4040
#define	FEISC_ERR_EMU2_TX_MISMATCH          -4041
#define	FEISC_ERR_EMU2_RX_SIZE              -4042
#define	FEISC_ERR_EMU2_READ_TESTCASE        -4043
#define	FEISC_ERR_EMU2_PARSE_TESTCASE       -4044
#define	FEISC_ERR_EMU2_CREATE_TESTCASE      -4045
...
// #####################################################
#define FEISC_THREAD_ID						1 // discontinued
#define FEISC_WND_HWND						2 // discontinued
#define FEISC_CALLBACK						3
#define FEISC_EVENT							4 // discontinued
#define FEISC_CALLBACK_2					5 // discontinued
...
#define FEISC_TASKCB_4						4 // only for FEISC_TASKID_SCANNER_EVENT
#define FEISC_TASKCB_NET_1				   11
#define FEISC_TASKCB_NET_4				   14 // only for FEISC_TASKID_SCANNER_EVENT
...
#define FEISC_TASKID_SCANNER_EVENT			8
...
// #####################################################
// structure for transfering event messages in callbacks
// #####################################################
...
typedef struct _FEISC_EVENT_INIT
{
	...
	unsigned int uiMsg;		// RFU
	unsigned int uiFlag;	// specifies the use of the union (e.g. FEISC_CALLBACK)
	union
	{
...
typedef struct _FEISC_TASK_INIT
{
	...
	union
	{
		...
		// only for FEISC_TASKID_SCANNER_EVENT
		void (FEISC_CALL *cbFct4)(	void* pAny,					// [in] pointer to anything (from struct _FEISC_TASK_INIT)
									int iReaderHnd,				// [in] reader handle of FEISC
									int iTaskID,				// [in] task identifier from FEISC_StartAsyncTask(..)
									int iError,					// [in] OK (=0), error code (<0) or status byte from reader (>0)
									unsigned char ucCmd,		// [in]	reader command
		...
		// only for FEISC_TASKID_SCANNER_EVENT
		void (CALLBACK *cbFctNET4)(	void* pAny,							// [in] pointer to anything (from struct _FEISC_EVENT_INIT)
									unsigned char ucCmd,				// [in]	reader command
									unsigned char* ucRspData,			// [in] response data
									int iRspLen,						// [in] length of response data
									char* cRemoteIP,					// [in] ip address of the reader
...
int  DLL_EXT_FUNC FEISC_0x3A_AdjustAutoReadMode(int iReaderHnd, unsigned char cBusAdr, unsigned char cMode, unsigned int uiDataSelector, unsigned int uiScanEngine);
...
int  DLL_EXT_FUNC FEISC_0xA1_RdLogin(			int iReaderHnd,
												unsigned char cBusAdr,
												unsigned char  cMode,
												const char* cUser, 
												int iUserLen, 
...
int  DLL_EXT_FUNC FEISC_0xA4_SetupReaderPassword(	int iReaderHnd,
													unsigned char cBusAdr,
													unsigned char  cMode,
													unsigned char  cPrivLevel,
													const char* cUser,
													int iUserLen,
													const char* cPw,
													int iPwLen);
...
int  DLL_EXT_FUNC FEISC_0xB5_ISOCustAndPropCmd( int iReaderHnd, 
												unsigned char cBusAdr, 
												unsigned char cMfr, 
												unsigned char* cReqData, 
												int iReqLen, 
